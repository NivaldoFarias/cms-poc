...
export default function Slug({ params, searchParams }: Props) {
  ...
  function Page() {
        ...
        <div className={formStyles.form_group}>
            ...
            {components[slug].map((component, index) => {
              return component.isSelect ? (
                <MultiSelect
                  ...
                  displayError={
                    displayError[
                      (component as SelectComponent).key as keyof DisplayError
                    ]
                  }
                />
              ) : (
                <InputSection
                  ...
                  displayError={
                    displayError[
                      (component as InputComponent).name as keyof DisplayError
                    ]
                  }
          ...
          <section className={formStyles.footer_section}>
            {!!groups_left ? (
              ...
              <button className={formStyles.next_btn} type={"submit"}>
                Cadastrar
              </button>
            )}
          </section>
    ...
    function handleChangeSelection(
      ...
      switch (action) {
        case "select-option": {
          const NewProvisions = (newValue as Option[]).map(
            ({ label, value }: Option) => {
              return { label, value };
            }
          );
    ...
    function parseHref() {
      const slug = groups_left.includes("-")
        ? groups_left.split("-")[0]
        : groups_left;
  ...
  async function registerUser() {
    const body =
      slug === "provisions"
        ? __normalizeProvisionsValue()
        : __normalizeDefaultValue();
    ...
    function __normalizeDefaultValue() {
      return {
        ...
        groups: {
          ...
          provisions: {
            type: (
              data.groups.provisions as unknown as {
    ...
    function __normalizeProvisionsValue() {
      return {
        ...
        groups: {
          ...
          provisions: {
            type: (
              form as unknown as { provisions: ProvisionsForms }
            ).provisions.map((provisions) => {
              return { value: provisions.label };
            }),
          },
